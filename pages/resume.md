...
### 万维日志分析系统
负责某些功能的开发工作。使用ruby on rails开发日志详情功能，使用elasticsearch的gem去获取es数据，利用ajax和es的scroll_id特性不断循环遍历出es某些索引里的全部数据，并以瀑布流的形式显示出来。使用python开发inotify脚本监控linux文件变化并以日志的形式保存到elasticsearch中
...
elastalert的docker化以及利用docker-compose部署运维运营部所需的所有日志分析系统；后期基于公司需求，使用harbor管理所有docker镜像，使用jenkins管理开发组前端实时push代码后编译docker镜像，并自动更新kubernetes中的前端服务；elasticsearch集群使用k8s管理，根据运营组需求部署master+data+ingest集群，存储使用ceph集群。
...
在校本科学习的是JAVA方向，主要学习利用ssh和spring mvc框架的javaweb开发，老师带着参加过网站的开发工作。
