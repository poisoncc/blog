...
思路：在aix上放一个shell脚本，添加定时任务，每五分钟运行一次，shell脚本会运行errpt指令，并抓取前五分钟的errpt日志，通过解析errpt的identifier，然后再运行`errpt -aj identifier | more`，最后把每一条errpt以及其more信息，组合成一个日志，然后通过logger伪装成系统日志，由syslog发送到远程的ELK中。
